...
!developmentChains.includes(network.name)
	...
	: describe("NftMarketplace Unit test", function () {
			let deployer, nftMarketplace, basicNft, txResponse, tokenId
			beforeEach(async () => {
			...
			describe("buyNft Tests", () => {
				it("Reverts if not already listed", async () => {
                    const price = ethers.utils.parseEther("1")
					await basicNft.approve(nftMarketplace.address, tokenId)
					await expect(nftMarketplace.buyNft(basicNft.address, tokenId,{value: price})).to.be.revertedWith("NftMarketplace__NotListed")
				})
				...
				it("Reverts if value sent is less", async () => {
					...
					await expect(nftMarketplace.buyNft(basicNft.address, tokenId,{value: price2})).to.be.revertedWith("NftMarketplace__NotEnoughPrice")
				})
				...
				it("Emits event", async () => {
					...
					const tx = await nftMarketplace.listNft(basicNft.address, tokenId, price)
					await tx.wait(1)
					await expect(nftMarketplace.buyNft(basicNft.address, tokenId,{value: price})).to.emit(nftMarketplace,"BoughtNft")
				})
			})
